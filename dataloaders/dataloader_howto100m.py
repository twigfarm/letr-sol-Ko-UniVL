...
class Youtube_DataLoader(Dataset):
        ...
        for token_id, token in enumerate(masked_tokens):
            ...
            if prob < 0.15:
                ...
                if prob < 0.8:
                    ...
                    masked_tokens[token_id] = random.choice(list(self.tokenizer.token2idx.items()))[0]
                try:
                    token_labels.append(self.tokenizer.token2idx[token])
                except KeyError:
                    token_labels.append(self.tokenizer.token2idx["[UNK]"])
            else:
                token_labels.append(-1)
    ...
    def _get_text(self, video_id, n_pair_max, sub_ids=None, only_sim=False, enhance_vmodel=False):
        ...
        for i in range(k):
            ...
            if only_sim is False:
                ...
                masked_token_ids = [self.tokenizer._convert_token_to_id(masked_token) for masked_token in masked_tokens]
    ...
    def _get_single_transcript(self, data_dict, ind, with_long_context=True):
        start, end = ind, ind
        words = self.tokenizer._tokenize(str(data_dict['text'][ind]))
        ...
        while with_long_context and (len(words) < self.min_words or diff < self.min_time):
            if start > 0 and end < len(data_dict['end']) - 1:
                next_words = self.tokenizer._tokenize(str(data_dict['text'][end + 1]))
                prev_words = self.tokenizer._tokenize(str(data_dict['text'][start - 1]))
                ...
                words = self.tokenizer._tokenize(str(data_dict['text'][start - 1])) + words
                start -= 1
            elif end < len(data_dict['end']) - 1:
                words.extend(self.tokenizer._tokenize(str(data_dict['text'][end + 1])))
                end += 1
            else:
                break
            diff = data_dict['end'][end] - data_dict['start'][start]
